...
<%
    ...
    String txt = "";
    if (!ToolsHTML.isEmptyOrNull((String)session.getAttribute("nodeType"))) {
        txt = rb.getString("security.toStruct" + session.getAttribute("nodeType")) + ": " + session.getAttribute("rout");
    }
    if (!Constants.cmdToStruct.equalsIgnoreCase((String)session.getAttribute("command"))) {
...
<script type="text/javascript">
   ...
   function updateCheck(check,field) {
    ...
    function deleteSecurity(idPerson,idUser) {
        ...
        if (idDocument!=null && idDocument!='null' && idDocument.length>0 ){
...
<body class="bodyInternas" <%=onLoad%>>
    ...
    <!-- Formulario para Obtener el nodo Seleccionado por el usuario -->
            <html:form action="/loadSecurityUser.do">
...
<!-- SIMON 09 JUNIO 2005 FIN -->
    <table border="0" width="100%">
        <tr>
            <td colspan="3" class="pagesTitle" style="font-size:20">
                <%= rb.getString("seguridad.title") + "&nbsp;" + txt.replaceAll(":",":<font size='-1'>") + "</font>"%>
            </td>
        </tr>
        <tr>
            <td>   
                <table class="clsTableTitle" width="100%" cellSpacing="0" cellPadding="2" align=center border=0>
                    <tbody>
                        <tr>
                            <td class="td_title_bc" height="21">User  
                                <%=rb.getString("seguridad.usersWithOutSec")%>
        ...
        <tr>
            <td valign="top">
                <table cellSpacing=0 cellPadding=0 align=center border=0 width="100%">
                    ...
                    <form name="busqueda" action="" onSubmit="return false;">
                        <input type="hidden" name="idNodeSelected" value='<%=session.getAttribute("idStruct")%>'/>
                        <tr>
                   ...
                   <!-- SIMON 09 JUNIO 2005 FIN -->
                    ...
                    indexId="ind" offset="<%=bean.getDesde()%>" length="<%=bean.getCuantos()%>">
                        ...
                        <tr>
                            <td class='td_<%=num%>'>
                            	<%if(String.valueOf(user.getIdGrupo()).equals(com.desige.webDocuments.utils.Constants.ID_GROUP_VIEWER)) {%>
                                    <%=user.getNameUser()%>
                                <%} else {%>
                        ...
                        <tr>
                            <td colspan="3" align="center">
                                ...
                                <form name="formPagingPage" method="post" action="allUsers.jsp">
                    ...
                    <!-- SIMON 9 DE JUNIO 2005 INICIO -->
                                       <tr>
                                            <td colspan="3" align="center">
                                                ...
                                                <form name="formPagingPage1" method="post" action="allUsers.jsp">
                                                    <input type="hidden" name="from"  value="">
                                                    <input type="hidden" name="cmd"  value="<%=SuperActionForm.cmdLoad%>">
                                                </form>
                                            </td>
