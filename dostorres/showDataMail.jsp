<!-- showDataMail.jsp -->
<%@ page import="java.util.ResourceBundle,
                 com.desige.webDocuments.persistent.managers.HandlerMessages,
                 com.desige.webDocuments.utils.beans.Users,
                 com.desige.webDocuments.utils.ToolsHTML"%>

<%@ taglib uri="/WEB-INF/struts-logic.tld" prefix="logic" %>
<%@ taglib uri="/WEB-INF/struts-template.tld" prefix="template" %>
<%@ taglib uri="/WEB-INF/struts-bean.tld" prefix="bean" %>
<%@ taglib uri="/WEB-INF/struts-html.tld" prefix="html" %>

<%@ page language="java" %>
<%
    ResourceBundle rb = ToolsHTML.getBundle(request);
    
    Users usuario = (Users)session.getAttribute("user");
	int mails = 0;
	if(usuario!=null){
		mails = HandlerMessages.getTotalInbox(usuario.getEmail());
	}
%>

<html>
<head>
<title><%=rb.getString("principal.title")%></title>
<jsp:include page="meta.jsp" /> 
<link href="estilo/estilo.css" rel="stylesheet" type="text/css">
<style>
.clsMessageView
{
  background: white;
  color: black;
  border: 0px solid #FFFFFF;
  padding: 2;
  font-size: 12px;
  font-family: Geneva, Verdana, Arial, Helvetica;
  font-weight: normal;
  background-color: transparent;
  /*overflow: scroll;*/
}
</style>
<script language="JavaScript">
    //parent.frames['superior'].location = "superior.jsp";

    function disabledBtns(forma) {
        forma.reSend.disabled = true;
        forma.reply.disabled = true;
        forma.cancel.disabled = true;
    }

    function cancelar(forma){
<%--        location.href = "loadInbox.do";--%>
        forma.action = "loadInbox.do";
        disabledBtns(forma);
        forma.submit();
    }
    function validar(forma){
        if (forma.to.value.length==0){
            alert ("<%=rb.getString("error.notTo")%>");
            return false;
        }
        if (forma.subject.value.length==0){
            alert ("<%=rb.getString("error.notSubject")%>");
            return false;
        }
        if (forma.mensaje.value.length==0){
            alert ("<%=rb.getString("error.notMessage")%>");
            return false;
        }
        return true;
    }
    function save() {
    }

    function send() {
    	forma = document.mails;
    	window.parent.parent.parent.frames['info'].nuevoMail(true);
        forma.subject.value = "RV: " + forma.subOrg.value;
        forma.to.value = "";
        forma.action = "loadMail.do";
        forma.target="bandeja";
        disabledBtns(forma);
        forma.submit();
    }

    function response() {
    	forma = document.mails;
    	window.parent.parent.parent.frames['info'].nuevoMail(true);
        forma.to.value = forma.from.value;
        forma.subject.value = "RE: " + forma.subOrg.value;
        //forma.mensaje.value = "";
        forma.action = "loadMail.do";
        forma.target="bandeja";
        disabledBtns(forma);
        forma.submit();
    }

</script>
<style>
.viewMessage
{
  background: white;
  color: black;
  border: 1px solid #FFFFFF;
  padding: 2;
  font-size: 12px;
  font-family: Arial, Helvetica, sans-serif;
  font-weight: normal;
  overflow: scroll;
}
</style>
</head>

<!--<body class="bodyInternas" style="margin:10px;background-color:afafaf;">-->
<body  style="margin:8px;background-color:868fa2;">
<div class="bodyInternas" style="border:1px solid #000000;padding:0px;">
<div style="margin:10px;">
    <logic:present name="sendMail" scope="session">
    <bean:define id ="mail" name="sendMail" type="com.desige.webDocuments.mail.forms.MailForm" scope="session"/>
        <form name="mails" action="" method="post">
			<html:hidden name="sendMail" property="to"/>
			<html:hidden name="sendMail" property="subject"/>
			<input type="hidden" name="subOrg" value ="<bean:write name='sendMail' property='subject'/>"/>
            <table align=center border="0" width="97%" bgcolor="#FFFFFF">
                <tr>
                    <td colspan="2" class="mailSubject">
                        <bean:write name="sendMail" property="subject"/>
                    </td>
                </tr>
                <tr>
                    <td colspan="2" class="mailFrom">
                        <bean:write name="sendMail" property="nameFrom"/>&nbsp;[<bean:write name="sendMail" property="from"/>]
                        <html:hidden name="sendMail" property="from"/>
                    </td>
                </tr>
                <tr>
                    <td class="mailTextColor" width="5%">
                        <%=rb.getString("mail.dateSended")%>:
                    </td>
                    <td class="mailText">
                        <bean:write name="sendMail" property="dateMail"/>
                    </td>
                </tr>
                <tr>
                    <td class="mailTextColor" valign="top">
                        <%=rb.getString("mail.to")%>:
                    </td>
                    <td class="mailText">
                        <%=mail.getTo().replaceAll(";","; ")%>
                    </td>
                </tr>
                <tr>
                    <td class="mailTextColor <%=ToolsHTML.isEmptyOrNull(mail.getCc(),"none")%>" valign="top" >
                        <%=rb.getString("mail.cc")%>:
                    </td>
                    <td class="mailText">
                        <%=ToolsHTML.isEmptyOrNull(mail.getCc(),"").replaceAll(";","; ")%>
                    </td>
                </tr>
                <tr>
                    <td colspan="2" class="mailFrom">
                    	<hr style="color:#3399FF;">
                    </td>
                </tr>
                
                <tr>
                    <td colspan="2" class="clsMessageView">
		                <%=mail.getMensaje()%>
                    </td>
                </tr>
                <tr class="none" >
                    <td colspan="4" align="center">
                        <input type="button" class="boton" value="<%=rb.getString("btn.forward")%>" name="reSend" onClick="javascript:send(this.form);" style="width:100px" />
                        &nbsp;
                        <input type="button" class="boton" value="<%=rb.getString("btn.reply")%>" name="reply" onClick="javascript:response(this.form);" style="width:100px" />
                        &nbsp;
                        <input type="button" class="boton" value="<%=rb.getString("btn.back")%>" name="cancel" onClick="javascript:cancelar(this.form);" style="width:100px" />
                    </td>
                </tr>
               <tr class="none">
                    <td colspan="4" align="left" style="padding: 0px" class="td_gris_l">
                        <div align="left" class="clsMessageView" id='mensaje' style="width: 100%; height: 160px">
                            <%=mail.getMensaje()%>
                        </div>
                    </td>
                    <html:hidden name="sendMail" property="mensaje"/>
               </tr>

            </table>
        </form>
    </logic:present>
</div>    
</div>    
</body>
</html>
<script language="javascript" event="onload" for="window">
	try{
	window.parent.parent.parent.frames['superior'].document.getElementById("mailwithoutreader").innerHTML="<%=mails%>";
	}catch(e){}

	if(window.parent.parent.parent.frames['info']){
		window.parent.parent.parent.frames['info'].returnMail(true);
	}
	
</script>