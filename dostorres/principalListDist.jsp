...
<script language="JavaScript">
	...
	function showDocumentPrint(idDoc,idVersion,nameFile,imprimir,idLogoImp){
		showDocumentPublishImp(idDoc,idVersion,nameFile,imprimir,idLogoImp);
		//setTimeout("window.open('showPublished.do','info')",3000);
		setTimeout("window.open('loadAllWF.do?goTo=reload','info')",2000);
	}
                ...
                <tr>
                    <td>
                        ...
                        <table width="100%" border="0" cellspacing="1" class="borderWindowPrincipal" >
                            <logic:present name="docVersionApproved" scope="session" >
                            	<% 
                            	    int cont=0;
                            	    Integer previousValue = 0;
                            	%>
                                <logic:iterate id="docCheck" name="docVersionApproved" type="com.desige.webDocuments.document.forms.DocumentsCheckOutsBean" scope="session" >
                                    <%
                                    ...
                                    <tr class='fondo_<%=cont==0?cont++:cont--%>'>
                                        ...
                                        <td width="60%" id="imgImpresora<%=docCheck.getIdDocument()%>">
                                            <!--<a href="javascript:showDocument('<%=docCheck.getIdDocument()%>')" target="info"  class="ahref_b">-->
                                            <a href="#" 
											  onclick="javascript:showDocumentPrint('<%=docCheck.getIdDocument()%>','<%=docCheck.getNumVer()%>','<%=docCheck.getIdDocument()%>','1','');"
                                              class="ahref_b">
                                            
                                                <%=docCheck.getNameDocument()%>
                                            </a>
                                        </td>
                                        <td class="txt_b" width="30%" align="center">
                                            <%=docCheck.getPrefix()+docCheck.getNumber()%>
                                        </td>
                                    </tr>
                                </logic:iterate>
                            </logic:present>
