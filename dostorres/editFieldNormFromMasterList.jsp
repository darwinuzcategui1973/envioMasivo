...
<script type="text/javascript" src="./estilo/funciones.js?mmm"></script>
...
<script type="text/javascript">
    ...
    function load() {
		...
		//	document.getElementById("ACTIVE_TD_"+idSeleccionados[i]).className="selectItemNow";
		//}
    }

   	function selectNormAndExit(idNorm) {
		...
		document.getElementById("ACTIVE_TD_"+idNorm).className="selectItemNow";
		closeWindow();
   	}
    ...
    function closeWindow(){
    	if(window.opener != null){
    		...
    		var idNorm = idSeleccionados[0];    		
    		var label = document.getElementById("ACTIVE_"+idNorm).innerHTML;
    		console.log(parentHiddenField+' - '+idNorm+' - '+label);
    		console.log(window.opener.setNormas);
    		window.opener.setNormaLabel(parentHiddenField,idNorm,label);
    		window.close();
    	}
    }
</script>
...
<body class="bodyInternas" style="margin: 0;">
							...
							<table cellSpacing="3" cellPadding="3" align=center border=0
								...
								<logic:iterate id="bean" name="dataTable" indexId="ind"
									...
									<tr class="<%=isMaster?"":idNormClass%>" <%= bean.getIndice().equals("0") ? "" : "style='display:none;'" %>>
										<td width="60%" class='td_<%=num%> selectItem' id='ACTIVE_TD_<%=bean.getId()%>' >
 ...
 %> 
 												<span id='ACTIVE_<%=bean.getId()%>'  onclick="selectNormAndExit(<%=bean.getId()%>)"><%=ToolsHTML.cortarCadena(bean.getDescript() != null ? bean.getDescript() : "")%></span>
