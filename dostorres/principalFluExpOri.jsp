                            ...
                            <logic:present name="wfExpiresOwner" scope="session" >
                                ...
                                <logic:iterate id="wfReq" name="wfExpiresOwner" type="com.desige.webDocuments.workFlows.forms.DataUserWorkFlowForm" scope="session" >
                                    ...
                                    <%
                                        ...
                                        if(! prevDocument.equals(wfReq.getIdDocument())){
                                        	cont = ((cont==0) ? ++cont : --cont);
                                        	prevDocument = wfReq.getIdDocument();
                                        	printValue = true;
                                        }else {
                                        	printValue = false;
                                        }
                                    %>
                                    <tr class='fondo_<%=cont%>'>
                                       <td width="6%">
                                            <img src="img/pendings.gif">
                                            <%=printValue ? wfReq.getIdVersion() : ""%>
                                        </td>
                                        <td width="45%">
                                            <a href="javascript:showWF('<%=wfReq.getIdWorkFlow()%>','<%=wfReq.getRow()%>',<%=wfReq.isOwner()%>,<%=wfReq.isFlexFlow()%>)" target="info" class="ahref_b">
                                                <%=printValue ? wfReq.getNameDocument()+" " +wfReq.getPrefix()+wfReq.getNumber() : ""%>
                                            </a>
                                        </td>
                                        <td class="txt_b" width="17%">
                                        <%--ydavila Ticket 001-00-003023--%>
                                        <%--<%=wfReq.getNameWorkFlow() : ""%>%>--%>
                                            <%=wfReq.getNameWorkFlow()%> <%=wfReq.getsubTypeWFdesc()%>
                                        </td>
                                        <td class="txt_b" width="17%" align="center">
                                            <%=printValue ? wfReq.getDateExpire(): ""%>
                                        </td>
                                        <td class="txt_b"  width="17%" align="center">
                                            <%=wfReq.getFlowRequieredByPersonBean().getApellido() + ", " + wfReq.getFlowRequieredByPersonBean().getNombre()%>
                                        </td>
                                    </tr>
                                </logic:iterate>
                            </logic:present>
