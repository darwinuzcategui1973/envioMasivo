package com.focus.qwds.ondemand.cliente.utils;
...
public class Encryptor {
	...
	public static String usuariosViewer = "20";
		...
		try {
			...
			for (int i = 0; i < providers.length; i++) {
				...
				if ("SunJCE".equals(name)) { 
					try {
						skf = javax.crypto.SecretKeyFactory.getInstance("DES", providers[i].getName());
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.print("Indique la cantidad de Licencias Viewer a Generar y pulse Enter: ");
			System.out.flush();
			usuariosViewer = entrada.readLine();
			int numeroViewer = Integer.parseInt(usuariosViewer);
			
			System.out.print("Indique la fecha de expiracion de la licencia en formato YYYYMMDD y pulse Enter: ");
			System.out.flush();
			String expireText = entrada.readLine();
			
			//System.out.println("Licencia para = " + usuarios + " usuarios ");
			String plaintext = usuarios;
			String plaintextViewer = usuariosViewer;
			
			String key = "16-171-151-56-196-97-64-7";
			String license = encrypt(key, plaintext);
			String licenseViewer = encrypt(key.concat("9"), plaintextViewer);
			String expire = encrypt(key, expireText);
			
			// GENERACION DE LICENCIA 4.5.X
			// COMENTADO PARA QUE NO SEA EJECUTADO EN CONSOLA
			/*
			System.out.println("license = " + license);
			System.out.println("license viewer = " + licenseViewer);
			System.out.println("license expire = " + expire);
			...
			System.out.println("prueba -> viewers = "+decrypt(key.concat("9"), licenseViewer));
			System.out.println("prueba -> viewers = "+decrypt(key, expire));
			*/
		} catch (Exception ex) {
