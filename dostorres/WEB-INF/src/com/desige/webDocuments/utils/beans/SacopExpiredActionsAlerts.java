...
/**
 * Clase para gestionar el envio de alertas automatico de acciones de SACOPs que están vencidas
 * 
 * @author frojas
 *
 */
public class SacopExpiredActionsAlerts extends TimerTask{
	private static final Logger log = LoggerFactory.getLogger(SacopExpiredActionsAlerts.class);
	...
	public SacopExpiredActionsAlerts() {
	...
	public void run() {
		...
		log.info("Inicio de envio de alertas de acciones SACOPS expiradas");
		try {
			...
			if(cal.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY
				...
				while (rs.next()){
					...
					String textMail = MessageFormat.format(bundle.getString("scp.mail.expiredactions.text"),
							new Object[]{rs.getString(6) + " " + rs.getString(7), rs.getString(4), rs.getString(1)});
					textMail += form.getMsgSacopAccionesPorVencer();
					
					String subject = MessageFormat.format(bundle.getString("scp.mail.expiredactions.subject"),
					...
					log.info("Enviado mail de accion expirada a [" 
		...
		log.info("Fin de envio de alertas de acciones SACOPS expiradas");
	}
}
