 ...
 *         </ul>
 */
public class FilesVersionAction extends SuperAction {
	private static Logger log = LoggerFactory.getLogger(FilesVersionAction.class.getName());
	...
	public synchronized ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (!ToolsHTML.isEmptyOrNull(request.getParameter("f1")) || !ToolsHTML.isEmptyOrNull(String.valueOf(request.getAttribute("f1")))) {
				...
				versiones = oExpedienteVersionDAO.findAllVersionGroup(files);

			}
			crs.beforeFirst();
			request.getSession().setAttribute("lista", crs);
			request.getSession().setAttribute("versiones", versiones);
			request.setAttribute("f1", files.getF1());
			return goSucces();
